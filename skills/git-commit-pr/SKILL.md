...
feature(worker): add webhook delivery routes
...
```text
feature(worker): add request audit events
...
Some repos use short scopes like `db`, `ui`, `worker`, `frontend`. Stricter repos may require full workspace scopes like `packages/db`, `apps/frontend`, or `@base/db`. Use the repo's convention. If the repo is ambiguous, prefer more specific scopes that identify the package or workspace, not just a broad area.

Subjects should be lowercase imperative, concise, and have no trailing period:

```text
feature(frontend): add saved view filters
fix(worker): reject missing webhook signatures
```
...
Commit 2: apps/worker routes and service wiring
...
<!--- Template hint only. Replace comments with final prose before publishing. Bullet list or brief summary of the main code changes, UI, worker, workflow changes, and blast radius. -->
...
- Each verification bullet should be concrete and brief, for example:
  ...
  - Asserted the order id is present in the worker response.
...
```bash
gh pr create --draft --base main --head "$(git branch --show-current)" --title "feature(worker): add webhook delivery management" --body-file /tmp/pr-body.md
```
